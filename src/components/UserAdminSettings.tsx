...
const UserAdminSettings = () => {
  ...
  React.useEffect(() => {
    ...
    if (savedUsers) {
      ...
      // Migração: Garante que a estrutura de sub-permissões financeiras existe
  ...
  const saveUsers = (updatedUsers: UserProfile[]) => {
    ...
    // Se o usuário editado for o atual, atualiza o currentUser no storage para refletir as mudanças na hora
                  ...
                  <TableRow key={user.id} className="hover:bg-blue-50/20 dark:hover:bg-slate-800/20">
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" className="text-blue-600" onClick={() => deleteUser(user.id)} disabled={user.role === 'ADMIN'}>
                          <Trash2 size={16} />
                        </Button>
                      </div>
                    </TableCell>
